...
# Encrypt secrets using GCP KMS
# Usage: echo "password" | ./encrypt.sh dev
# Usage: echo "password" | ./encrypt.sh -c 80 prod
...
if [ $# -ne 1 ]; then
    echo "Usage: $0 [-c <cols>] <env>"
    echo ""
    echo "Encrypts stdin using GCP KMS and Base64 encodes output"
    echo ""
    echo "Arguments:"
    echo "  env    Environment: dev, cert, or prod"
    echo ""
    echo "Options:"
    echo "  -c     Base64 column width (default: 64)"
    echo ""
    echo "Example:"
    echo "  echo 'MyPassword' | $0 dev"
    echo "  echo 'MyPassword' | $0 -c 80 prod"
    exit 1
fi
